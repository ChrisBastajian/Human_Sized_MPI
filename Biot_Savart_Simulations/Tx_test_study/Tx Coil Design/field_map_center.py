"""
Average breast size in 34DD which corresponds to a 5 inch underbust to bust difference. The
average diameter falls between 11 and 15 centimeters.
This script is based on these measurements to design a coil that fits 50% bigger/ smaller breasts.
"""
...
R = 15 * 1e-2 * 1.5 # the radius of the curve itself (not the distance for each vector) in m
mu0 = 4 * np.pi * 1e-7
I = 10 #current intensity in A
n_turns = 20*4
max_height = 12.7 * 1e-2 *1.5
...
a = coil_arrays(num_layers, coil_thickness)
...
#plot everything
lx_flat = np.concatenate([lx[str(i)] for i in range(num_layers)])
ly_flat = np.concatenate([ly[str(i)] for i in range(num_layers)])
lz_flat = np.concatenate([lz[str(i)] for i in range(num_layers)])
